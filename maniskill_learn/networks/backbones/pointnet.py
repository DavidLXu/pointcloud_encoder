...
from maniskill_learn.networks.modules.activation import build_activation_layer
from maniskill_learn.utils.data import dict_to_seq
from maniskill_learn.utils.torch import masked_average, masked_max
...
class SparseUnetV0(PointBackbone):
    def __init__(self, conv_cfg, mlp_cfg, stack_frame=1, subtract_mean_coords=False, max_mean_mix_aggregation=False, with_activation=False):
        ...
        from sparseunet import  SparseUnetBackbone
...
from maniskill_learn.utils.meta import build_from_cfg
...
class PointNetWithInstanceInfoV0(PointBackbone):
	...
	def forward(self, data):
		...
		pcd = {'xyz': data["pc"][:, :, :3], 'seg': data["pc"][:, :, 3:3+self.mask_dim]}
		state = data["state"]
